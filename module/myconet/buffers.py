...
def rearrange_feature_map_output(outputs):
    """ Converts a feature map output into a single buffer list object"""
    input_gradients = []
    weight_gradients = []
    bias_gradients = []
...
class BufferList:
    ...
    def get_as_buffer(self, index=None):
        """ Return a single buffer at a given index. If an index is not given, return every buffer concatenated together (As a GPU Buffer)"""
        if index is None:
