...
def load_training_data(mutations_per_image=20, transformations_per_image=3, pre_load=False, cache=True):
    trainingData = []
    transformations = (rotate_180, blur, colour_shift, add_noise_specs, random_colour_shift)

    with open(".cache/data_maker/training_data_hash.txt", "rb") as f:
        old_hash = f.read()
    ...
    with open("trainingPoints.txt", "r") as f:
        ...
        if cache and file_hash == old_hash and os.path.exists(".cache/data_maker/training_data_cache.bin"):
            return read_from_cache(".cache/data_maker/training_data_cache.bin")
    ...
    if pre_load:
        ...
        if cache and (file_hash != old_hash or not os.path.exists(".cache/data_maker/training_data_cache.bin")):
            print("[PreLoading] Writing loaded data to cache...")
            write_to_cache(".cache/data_maker/training_data_cache.bin", trainingData)

            with open(".cache/data_maker/training_data_hash.txt", "wb") as fb:
                fb.write(file_hash)
